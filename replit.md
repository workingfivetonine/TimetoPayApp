...
**Email reminders (Loops, optional):** `lib/notifications/reminders.ts` sends 5 reminder types. Each fires as a Loops EVENT — the email body is authored in the Loops dashboard against the event name, so a type whose template doesn't exist yet is a silent no-op. (This section previously described a Resend connector rendering HTML in `lib/email/templates.ts`; that is not how it works — everything goes through Loops over plain HTTPS with `LOOPS_API_KEY`.)
- Types: `trial_ending` and `past_due` (critical, sent regardless of marketing opt-out, each dedupes via its own user-level cursor); then three opt-in engagement types gated on `notifyListExport` / `notifyReceiptReminders` / `notifySpendSummary`, all of which default to **false**.
- `notifyReceiptReminders` covers two triggers: generic inactivity (`receipt_inactivity`), and `trip_receipt_missing` — fired a week after a Shopping Mode trip closes when no receipt has been logged since. The trip-anchored one is tried first and suppresses the inactivity nudge in the same sweep, so a user can't get two near-identical "upload a receipt" emails at once.
- **Eligibility:** any user with an email, past a 2-day signup grace. This used to require real billing status (entitled or past_due), which silently killed every engagement reminder once the app went free — almost nobody has a subscription, so almost nobody reached the per-type checks. Opt-in is still enforced per type.
- Cadence: `REMINDER_INTERVAL_MS` (default hourly), `REMINDER_INITIAL_DELAY_MS` (default 90s). Cursors only advance on a successful send, so reminders resume once `LOOPS_API_KEY` is configured.
...
- `artifacts/api-server/src/lib/email/` — `resendClient.ts` (Resend `POST /emails` sender), `templates.ts` (code-rendered email subject/HTML/text), `gmailClient.ts` (admin digest); all connector-backed, graceful no-op
...
### Auth & admin
- Replit-managed Clerk; every user's data is private (`userId` scoped per request by `requireAuth`). Expo client uses `@clerk/expo` bearer tokens (web too); `setAuthTokenGetter` attaches the token to every generated call. Sign-out/user-switch clears the React Query cache. Clerk keys auto-provisioned (`CLERK_PUBLISHABLE_KEY` → `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`) — never ask the user. Self-service password reset is user-initiated on the sign-in screen. Clerk has no admin "send reset email" API, but an admin CAN force one: `POST /admin/users/:userId/force-password-reset` calls `clerkClient.users.setPasswordCompromised(userId, { revokeAllSessions: true })`, so Clerk demands a new password at next sign-in. No email is sent — the admin has to tell the user out of band — and it's a no-op for Google-only accounts with no password.

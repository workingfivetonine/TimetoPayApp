...
**Email reminders (Resend connector, optional):** sends 4 reminder types to subscription-related users. Resend has NO dynamic-template-id concept — each email's subject + HTML + text is rendered in code (`lib/email/templates.ts`), so email design is version-controlled, not dashboard-edited.
- `RESEND_FROM_EMAIL` (required to send, verified sender/domain in Resend), `RESEND_FROM_NAME` (default `Receipt Tracker`). Sent via `POST /emails` through the Replit `resend` connector proxy (no manual API key).
- Cadence: `REMINDER_INTERVAL_MS` (default hourly), `REMINDER_INITIAL_DELAY_MS` (default 90s). No-ops if Resend unconfigured (connector unbound or `RESEND_FROM_EMAIL` unset); per-type cursor only advances on successful send, so reminders resume once configured.

...
- Additive schema changes are applied at server boot via idempotent `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` / `CREATE TABLE IF NOT EXISTS` in `bootstrap.ts`. There is also a `lib/db/drizzle/` migration folder, so column *drops* are a deliberate `drizzle-kit` step, not automatic.
...
- **Shopping Mode trips**: a trip ends ONLY on the explicit "Done shopping" button — switching sub-tabs isn't leaving the screen, and backgrounding mid-shop is normal, so nothing else may fire a reminder the user didn't earn. Closing POSTs to `/shopping-list/trips` (fire-and-forget) and starts the `trip_receipt_missing` clock. Per-trip `picked` state is intentionally not persisted.
